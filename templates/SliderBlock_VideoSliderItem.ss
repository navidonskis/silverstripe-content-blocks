<div data-item="video" class="slider-block__video-slider<% if $SliderImage %> slider-block__has-background<% end_if %>"<% if $SliderImage %> style="background-image: url('$SliderImage.SetWidth(1600).URL');"<% end_if %><% if $EmbedLink %> data-embed="$EmbedLink"<% end_if %><% if $AutoPlay %> data-autoplay="$AutoPlay"<% end_if %><% if $VideoType %> data-videotype="$VideoType"<% end_if %>>
    <div class="slider-block__overlay">
        <span class="slider-block__overlay--play-button">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" viewBox="0 0 314.068 314.068" xml:space="preserve">
